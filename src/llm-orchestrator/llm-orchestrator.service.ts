...
export class LlmOrchestratorService {
  ...
  async personalizeGoals(
    ...
    const prompt = [
      ...
      '   Use null for anything the profile does not actually measure. A key describing',
      '   what the user has ALREADY achieved (months completed, streaks, amounts saved',
      '   so far) must never be filled with the goal\u2019s target number — that would tell',
      '   the user they are finished when they have not started. When unsure, use null.',
