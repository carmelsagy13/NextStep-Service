...
export class OpenFinanceService {
  ...
  private async callLlmForStateAndReconciliation(
    ...
    const systemPrompt = [
      ...
      'History is TREND CONTEXT ONLY — it is NOT a floor and must never lock the',
      'level in place. The new Open Finance summary is authoritative and MAY move the',
      'level DOWN as well as up: when current signals show genuine deterioration (e.g.',
      'loanVSaffordability > 1, rising deficitMonthsCount, non-zero systemFlags), apply',
      'the downgrade even if the previous assessment sat at a higher step.',
      '',
      ...
      '- For RECURRING / ongoing params (e.g. monthly_deficit, a monthly saving or surplus figure), base them on the MULTI-MONTH AVERAGE (avgMonthlyIncome / avgMonthlyExpense, deficitMonthsCount over monthsCovered, monthlyNetCashFlow), NOT a single month. A single positive month does NOT make the deficit 0 when the ongoing average gap is negative — use the real average gap.',
      '',
      ...
      '(For trend context only — never a floor; the current summary may justify a lower step.)',
      historySection,
      '',
      '## Existing Tasks',
